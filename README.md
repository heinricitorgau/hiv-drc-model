...
- **99 tests**, no test comparing the code to itself.
...
Run everything and write all eight figures to `figures/`:

```bash
python -m hiv_drc
```

The five stages are `dynamics`, `r0`, `stability`, `sensitivity` and `estimate`. Individual
stages can be selected by name:
...
All eight are written to `figures/` (git-ignored — they are regenerated by one command).
...
99 tests, roughly 9 seconds. The design principle is that **no test compares the code to
...
├── tests/                   92 tests (99 with doctests)
